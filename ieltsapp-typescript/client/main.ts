...
interface SkippedQuestionGroup {
  numbers: string;
  type: string;
  reason: string;
}
...
interface PracticeSet {
  ...
  answers_inferred?: boolean;
  skipped_questions?: SkippedQuestionGroup[];
...
const elements = {
  ...
  importPdfBtn: byId<HTMLButtonElement>('importPdfBtn'),
  importPdfInput: byId<HTMLInputElement>('importPdfInput'),
  importNotice: byId<HTMLElement>('importNotice'),
...
function readFileAsPdfDataUrl(file: File): Promise<string> {
...
async function pollJobStatus(jobId: string, options: { label?: string; maxDuration?: number } = {}): Promise<PracticeSet | null> {
...
function fitbQuestionsOf(practiceSet: PracticeSet): FitbQuestion[] {
  return (practiceSet.questions ?? []).filter((question): question is FitbQuestion => question.question_type === 'FITB');
}

function tfngQuestionsOf(practiceSet: PracticeSet): TfngQuestion[] {
  return (practiceSet.questions ?? []).filter((question): question is TfngQuestion => question.question_type === 'TFNG');
}

function hasMatchingHeadings(practiceSet: PracticeSet): boolean {
...
function renderImportNotice(practiceSet: PracticeSet): void {
...
function renderMixedQuestions(practiceSet: PracticeSet): void {
...
function renderMatchingHeadings(practiceSet: PracticeSet): void {
